...
namespace GraphUtils {
    ...
    float diffGray[] = { 0.5f, 0.5f, 0.5f };
    float ambiGray[] = { 0.1f, 0.1f, 0.1f };
    float specGray[] = { 0.7f, 0.7f, 0.7f };
    ...
    void drawText(const char* textCube, float x, float y, float z)
    {
        glRasterPos3f(x, y, z);

        for (int i = 0; textCube[i] != '\0'; ++i)
        {
            glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, textCube[i]);
        }
    }
